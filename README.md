...
- **이수찬**: 프로젝트 총괄, DB 설계, 시스템 구조 설계, Spring 백엔드 구현, CI/CD 구성, 프론트 통신 API 로직 간략화, AWS 배포, 코드 보수
...
cocaBack/
    ├── src/main/java/project/coca
        ...
        ├── schedule    # 일정 관리 패키지
        ├── tag         # 태그 관련 패키지
        ├── HealthCheckController.java      # ALB 상태 검사용 클래스
        ├── InitData.java                   # 시작 데이터 구성 클래스
        ├── JwtSecurityConfig.java          # Jwt 설정 클래스
        ├── RedisConfig.java                # Redis 설정 클래스
...
혹은 [COCA 시스템 개발 리뷰](https://velog.io/@lsc4814/COCA-v1-%EA%B5%AC%ED%98%84-%EC%8B%9C%EC%8A%A4%ED%85%9C-%EB%A6%AC%EB%B7%B0)
를 참고해주세요
